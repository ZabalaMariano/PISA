...
#include "./indexed_sequence_v2.hpp"
...
namespace pisa {
    ...
    struct partitioned_vb_sequence {
        ...
        {
            assert(n > 0);
            auto partition = pvb::optimizer<VByteBlockType>::compute_partition(begin, n, conf);
            ...
            if (partitions == 1) {
                ...
                compact_elias_fano::write(bv_sizes, sizes.begin(),
                                          n, partitions - 1, params, conf);
                compact_elias_fano::write(bv_upper_bounds, upper_bounds.begin(),
                                          universe, partitions + 1, params, conf);
    ...
    private:
        ...
        static const uint64_t type_bits = indexed_sequence_v2<>::type_bits;

        template<typename Iterator>
        static void write_block(bit_vector_builder& bvb,
