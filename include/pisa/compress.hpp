    ...
    size_t postings = 0;
    {
        ...
        if (quantized_scorer) {
            ...
            for (auto const& plist: input) {
                ...
                builder.add_posting_list(size, plist.docs.begin(), quantized_scores.begin(), sum, conf, index_encoding);
                term_id += 1;
            ...
            for (auto const& plist: input) {
                ...
                builder.add_posting_list(size, plist.docs.begin(), plist.freqs.begin(), freqs_sum, conf, index_encoding);
                progress.update(1);
                postings += size;
                term_id += 1;
            }
    ...
    std::string const& index_encoding)
{
    if constexpr (std::is_same_v<typename CollectionType::index_layout_tag, BlockIndexTag>) {
        ...
        compress_index_streaming<CollectionType, WandType>(
            input, params, *output_filename, std::move(quantized_scorer), check, conf, index_encoding);
        return;
    }
    ...
    size_t postings = 0;
    {
        ...
        for (auto const& plist: input) {
            ...
            if (quantized) {
                ...
                builder.add_posting_list(size, plist.docs.begin(), quants.begin(), quants_sum, conf, index_encoding);
            } else {
                uint64_t freqs_sum =
                    std::accumulate(plist.freqs.begin(), plist.freqs.begin() + size, uint64_t(0));
                builder.add_posting_list(size, plist.docs.begin(), plist.freqs.begin(), freqs_sum, conf, index_encoding);
